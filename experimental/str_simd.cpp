...
void prog_p_successor(const int pindex, simd_uint8 &lengths, simd_uint8 &bits,
                      std::vector<int> &levels) {
  simd_uint8_mask has_bits = (lengths > 0);
  simd_uint8 nlb_counts{std::popcount(static_cast<uint8_t>(lengths[0])) -
                        has_bits[0]}; /*([lengths](int8_t n) {
return static_cast<int8_t> (std::popcount(static_cast<uint8_t>(lengths[n])));
});*/
  for (size_t n = 1; n < lengths.size(); n++) {
    // Add the NLB in this to the previous result, subtract the leading bit if
    // there is one
    nlb_counts[n] = nlb_counts[n - 1] +
                    std::popcount(static_cast<uint8_t>(lengths[n])) -
                    has_bits[n];
  }

  simd_uint8_mask smaller_than_p =
      simd_uint8([&levels](uint8_t i) {
        return i < levels.size() ? levels[i] : 0b11111111;
      }) <= simd_uint8{pindex};
  simd_uint8 clear_first_bit(~simd_uint8{0} & ~1);
  simd_uint8 pattern_zero_and_ones = lengths & clear_first_bit;
  simd_uint8 strings_after(
      [&levels](uint8_t i) { return static_cast<uint8_t>(levels.size() - i); });
  simd_uint8 needed_after([&levels](uint8_t i) {
    return i < levels.size() ? (h - levels[i] - 1) : 0;
  });
  simd_uint8 nlb_before(
      [&nlb_counts](uint8_t i) { return i == 0 ? 0 : nlb_counts[i - 1]; });
  simd_uint8_mask no_successor =
      has_bits and
      ((nlb_before == t) or
       ((nlb_counts == t) and (((bits == pattern_zero_and_ones) and
                                (strings_after == needed_after)) // Third case
  ...
  if (match == -1) {
    ...
    int reset_bits =
        std::countl_one(static_cast<uint8_t>(bits[match] | ~lengths[match])) +
        1;
    ...
    if (reset_bits < 8) {
      nlb_counts[match] -=
          current_bits - std::popcount(static_cast<uint8_t>(lengths[match]));
      match++;
    } else {
      // The new string is empty...
      nlb_counts[match] -= (current_bits - 1);
      if ((std::cmp_equal(match + 1, levels.size()) and
           levels[match] < h - 2) or
          (std::cmp_less(match + 1, levels.size()) and
           levels[match] + 1 < levels[match + 1])) {
        // Empty level!
        levels[match] = std::cmp_equal(match + 1, levels.size())
                            ? levels[match] + 1
                            : levels[match + 1] - 1;
      } else
        levels[match]++;
    ...
    if ((std::cmp_less(match + 1, levels.size()) and
         levels[match] + 1 < levels[match + 1] and levels[match] != p)) {
      // There is an empty level we can use!
      match++; // We append to the next string
      bits[match] = 1;
      levels[match] = std::min(
          p, levels[match] -
                 1); // Move the empty string up to the next higher level
    } else {
  ...
  if (match < k - 1) {
    ...
    simd_uint8 after_match([match](uint8_t i) { return i > match; });
    simd_uint8 before_levels(
        [&levels](uint8_t i) { return i < levels.size(); });
    stdx::where((before_levels > 0 and after_match > 0), lengths) = 1;
    stdx::where((after_match > 0), bits) = 0;
  }
}

void parse_to_vec(size_t idx, std::vector<uint8_t> &bits,
                  std::vector<uint8_t> &lengths, std::vector<int> &levels) {
  bits.clear();
  lengths.clear();
  levels.clear();
  ...
  for (size_t i = 0; i < entry_b.size(); i++) {
    ...
    if (entry_b[i])
      bits[under_construction] |= (1 << current_bit);
    current_bit++;
  }
...
int compare(const int pindex, size_t idxA, size_t idxB) {
  ...
  simd_uint8 first_length_difference =
      (lengths_A ^ lengths_B) & ~((lengths_A ^ lengths_B) * 2);
  simd_uint8 bit_xor = bits_A ^ bits_B;
  simd_uint8_mask a_less =
      ((lengths_A < lengths_B) and
       (bit_xor & (shorter_string + first_length_difference)) ==
           first_length_difference) or
      ((lengths_A > lengths_B) and
       (bit_xor & (shorter_string + first_length_difference)) == 0);
  simd_uint8_mask b_less =
      ((lengths_B < lengths_A) and
       (bit_xor & (shorter_string + first_length_difference)) ==
           first_length_difference) or
      ((lengths_B > lengths_A) and
       (bit_xor & (shorter_string + first_length_difference)) == 0);
  ...
  simd_uint8_mask a_greater =
      (different_bits > 0) and ((bits_A & first_bit_difference) > 0);
  simd_uint8_mask b_greater =
      (different_bits > 0) and ((bits_B & first_bit_difference) > 0);
...
int main() {
  // Set for initial bitstring: k-1 bitstrings with t NLB in the first one, all
  // 0s
  ...
  for (size_t i = 0; i < tree_b.size() - 1; i++) {
    ...
    if (levels == succ_levels and bits_vec == succ_bits and
        lengths_vec == succ_lengths) {
    ...
    std::cout << i << ": " << fwd_res;
    if (fwd_res == -1)
      std::cout << " \033[32mCorrect forward comparison!\n\033[0m";
    else {
    ...
    std::cout << i << ": " << bwd_res;
    if (bwd_res == 1)
      std::cout << " \033[32mCorrect backward comparison!\n\033[0m";
    else {
