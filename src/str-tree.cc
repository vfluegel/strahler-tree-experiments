...
void getLevelPSuccessor(int idx, int p, std::vector<std::vector<bool>> &use_b,
                        std::vector<std::vector<int>> &use_d) {
  tmp_b = use_b[idx];
  tmp_d = use_d[idx];
  ...
  // Calculate number of Non-Empty Strings (NES): count unique values in tmp_d
  // up until there
  size_t nes =
      std::unordered_set<int>(tmp_d.begin(), tmp_d.begin() + i + 1).size();
  std::cout << nes << std::endl;
  // Subtract the string that we currently look at (A, B only refers to strings
  // "above")
  if (std::cmp_equal(
          nes, k)) { // we actually want to check nes - 1 == k - 1, but we can
  ...
  while (i >= 0) {
    ...
    if (std::cmp_equal(i + 1, tmp_d.size()) or tmp_d[i] != tmp_d[i + 1]) {
      ...
      // Calculate the Non-Leading Bits (NLB): take the complete length and
      // subtract the number of NES (every NES has one leading bit)
      int nlb = (i + 1) - nes;
      nes--;
      ...
      if (nlb < t) {
        ...
        int new_index =
            std::cmp_equal(i + 1, tmp_d.size()) ? tmp_d[i] : tmp_d[i + 1] - 1;
        i++;

        if (i != 0 and tmp_d[i - 1] == tmp_d[i])
          nlb++;
        else
          nes++;
      ...
      // Case B: Check if the current string is only leading bit (so all NLB are
      // in strings 0 to r-1)
    ...
    if (tmp_b[i] == 0) {
      if (i == 0 || tmp_d[i - 1] != tmp_d[i]) {
        ...
        // The 0 is either the first bit in total, or it is the first bit of
        // that level
        int strings_after_current =
            std::unordered_set<int>(tmp_d.begin() + i, tmp_d.end()).size();
        if (strings_after_current == h - tmp_d[i]) {
          // All bitstrings after the current level are non-empty, we simply
          // move on C: No sibling on this layer
          i--;
          continue;
        } else {
          // F: We have to remove one NES: The string is 01^j, so the new string
          // would be empty
          nes--;
          skipLevel = true;
        }
        // We can use this level: Break out of the loop and start changing at
        // the current i
        break;
      } else {
      ...
      // We can already start setting everything there to 0 and change where it
      // belongs later
      tmp_b[i] = 0;
      i--;
    }
  ...
  if (i == -1) {
    if (tmp_d[0] == 0) {
      ...
      // Special case: There are still empty strings "left" that can be filled
      // instead
      i = 0;
      tmp_b[i] = 1;
      // TODO: maybe find more elegant way to avoid setting the index too high
      // as opposed to starting out one lower...
      tmp_d[i] = tmp_d[i] - 2;
      skipLevel = true;
      nes--;
    }
  ...
  if (no_of_needed_nes == 0) {
    // special case: Don't add anything, remove everything after the current
    // position
    tmp_d.resize(i);
    tmp_b.resize(i);
  } else {
  ...
  while (i >= 0) {
    ...
    if (std::cmp_equal(i + 1, tmp_d.size()) or tmp_d[i] != tmp_d[i + 1]) {
      ...
      if (nlb < t) {
        ...
        // Needed length: after current position, missing NLBs + NES so k-1 or
        // height
...
#ifndef NDEBUG
          if (trace >= 2)
            std::cout << "Resizing to fit bits (" << newBits
                      << " additional bits)\n";
#endif
          tmp_b.insert(tmp_b.end(), newBits, 0);
          tmp_b[i] = 1;
          for (size_t j = i; j < tmp_d.size(); j++)
            tmp_d[j] = new_index;
          tmp_d.insert(tmp_d.end(), newBits, new_index);
          i += t - nlb + 1;
          std::cout << "Resized tmp: now i = " << i << ", NES = " << nes
                    << std::endl;
        } else {
#ifndef NDEBUG
          if (trace >= 2)
            std::cout << "Adding bits\n";
#endif
          tmp_b[i] = 1;
          tmp_d[i] = new_index;
    ...
    if (tmp_b[i] == 0) {
      if (i == 0 || tmp_d[i - 1] != tmp_d[i]) {
        ...
        int strings_after_current =
            std::unordered_set<int>(tmp_d.begin() + i, tmp_d.end()).size();
        std::cout << "Strings after current: " << strings_after_current
                  << " of " << (h - 1) << " - " << tmp_d[i] << std::endl;
...
// Change where the bits belong
  ...
  // We can only fill as many NES as we have height... Too many empty strings
  // "in the middle" = less tham k-1 strings total
...
#ifndef NDEBUG
    if (trace >= 2)
      std::cout << "Needed nes: " << no_of_needed_nes
                << ", available levels: " << available_nes << std::endl;
#endif
    // Fill up with just the next one as long as we still have bits
    while (std::cmp_greater_equal(tmp_b.size(), i + no_of_needed_nes)) {
      assert(i >= 0 and std::cmp_less(i, tmp_d.size()));
...
#ifndef NDEBUG
      if (trace >= 2)
        std::cout << "Set bit " << i << " to " << set_index << std::endl;
#endif
      tmp_d[i] = set_index;
      i++;
    }
...
int compare(size_t idxA, size_t idxB, int pindex,
            std::vector<std::vector<bool>> &use_b,
            std::vector<std::vector<int>> &use_d) {
  ...
  // cases involving Top
  if (indicesA[0] == -1 and indicesB[0] == -1)
    return 0;
  if (indicesA[0] == -1)
    return 1;
  if (indicesB[0] == -1)
  ...
  if (argc == 1) {
    for (size_t idx = 0; idx < use_b.size(); idx++) {
      ...
      if (((idx == use_b.size() - 1 and tmp_d[0] == -1)) or
          (tmp_b == use_b[p_successors[idx]] and
           tmp_d == use_d[p_successors[idx]])) {
